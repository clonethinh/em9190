...
detect_device() {
    local potential_devices=""
    
    # 1. Check UCI configuration first (e.g., from 3ginfo or modemdefine packages)
    local uci_device=""
    if command -v uci > /dev/null 2>&1; then
        uci_device=$(uci -q get 3ginfo.@3ginfo[0].device 2>/dev/null)
        if [ -n "$uci_device" ] && [ -e "$uci_device" ]; then
            potential_devices="$potential_devices $uci_device"
        fi
        # Check modemdefine configurations if present
        uci_device=$(uci -q get modemdefine.@modemdefine[0].comm_port 2>/dev/null)
        if [ -n "$uci_device" ] && [ -e "$uci_device" ]; then
             potential_devices="$potential_devices $uci_device"
        fi
         uci_device=$(uci -q get modemdefine.@general[0].main_modem 2>/dev/null)
        if [ -n "$uci_device" ] && [ -e "$uci_device" ]; then
             potential_devices="$potential_devices $uci_device"
        fi
    fi

    # 2. Look for common modem device patterns in /dev
    # Sort in reverse to prioritize devices like ttyUSB2 over ttyUSB0 (often newer ones are listed last)
    # Also include common WWAN AT ports
    potential_devices="$potential_devices $(find /dev -name "ttyUSB*" -o -name "ttyACM*" -o -name "wwan*at*" | sort -r)"
    
    # 3. Filter by WAN device if available
    local wan_device_path=""
    if [ -n "$WAN" ] && [ -e "$WAN" ]; then
        # Get the kernel device path for the WAN interface
        # This is a bit complex and might vary. A simpler approach is to just check if the device
        # is already configured as WAN, but that might be too restrictive.
        # For now, we'll rely on AT command tests.
        : # Placeholder, as getting the actual /dev/ path from WAN interface name is tricky
    fi

    # Deduplicate and clean up potential devices
    potential_devices=$(echo "$potential_devices" | tr ' ' '\n' | sort -u | grep -v '^$' | grep -E '/dev/tty(USB|ACM|S0)[0-9]+|/dev/wwan[0-9]+at')

    # Test each potential device
    for DEVICE in $potential_devices; do
        if [ -e "$DEVICE" ]; then
            # Use sms_tool if available, otherwise gcom
            local tool_cmd=""
            if command -v sms_tool >/dev/null 2>&1; then
            ...
            if [ -n "$tool_cmd" ]; then
                ...
                if timeout 3 sh -c "$tool_cmd" >/dev/null 2>&1; then
                    echo "$DEVICE"
                    # Optionally, save this to /tmp/modem for future use
...
# Note: This assumes detect_device is sourced or available in the PATH.
# For simplicity and self-containment, we'll embed its logic here or source it.
# Sourcing the script's detect_device function:
. "$INSTALL_DIR/scripts/detect_device.sh" # Assuming detect_device function is in this sourced file
    ...
    "reset")
        ...
        if [ -n "$DEVICE" ]; then
            ...
            if command -v sms_tool >/dev/null 2>&1; then
                ...
                reset_cmd="gcom -d '$DEVICE' -s /usr/share/3ginfo-lite/reset.gcom" # Assuming reset.gcom exists for gcom
            fi
...
# --- Tạo Script phát hiện thiết bị modem (for API) ---
# This script will be sourced by api.cgi to provide the detect_device function.
echo "🛠️ Tạo script phát hiện thiết bị modem..."
cat > "$INSTALL_DIR/scripts/detect_device.sh" << 'EOF'
#!/bin/sh
# Provides the detect_device function for API scripts.
    ...
    for DEVICE in $potential_devices; do
        if [ -e "$DEVICE" ]; then
            ...
            if command -v sms_tool >/dev/null 2>&1; then
                ...
                # Assuming /usr/share/3ginfo-lite/check.gcom is available and works for a basic AT test
            ...
            if [ -n "$tool_cmd" ]; then
                ...
                if timeout 3 sh -c "$tool_cmd" >/dev/null 2>&1; then
                    echo "$DEVICE"
                    # Optionally, save this to /tmp/modem for future use by other scripts
                    echo "$DEVICE" > /tmp/modem
                    return 0
                fi
            fi
